...
namespace stringSplit {
    ...
    vector <string>
    Split(const string &strIn, const string &strDelimiters, bool bRemoveEmpty = false, bool bMatchAll = false) {
        ...
        while (SearchStart != string::npos) {
            string::size_type SearchEnd = (strIn.*FindPosition)(strDelimiters,
                                                                SearchStart);    // get a complete range [SearchStart..SearchEnd)
            ...
            if (!(bRemoveEmpty && strToken.empty())) {
                VecStrTokens.push_back(
                        strToken);                  // extract the strToken and add it to the VecStrTokens list
            }
