...
void CConsoleHelper::CreateConfigOptions(CONFIG_OPTIONS *CfgOptions, string strCfg, CDP5Status DP5Stat,
...
void CConsoleHelper::ProcessSpectrumEx(Packet_In PIN, DppStateType DppState) {
    ...
    for (idxSpectrum = 0; idxSpectrum < DP5Proto.SPECTRUM.CHANNELS; idxSpectrum++) {
        DP5Proto.SPECTRUM.DATA[idxSpectrum] =
                (long) (PIN.DATA[idxSpectrum * 3]) + (long) (PIN.DATA[idxSpectrum * 3 + 1]) * 256 +
                (long) (PIN.DATA[idxSpectrum * 3 + 2]) * 65536;
    }
...
void CConsoleHelper::ProcessListModeDataEx(Packet_In PIN, DppStateType DppState) {
    static unsigned long long data_number = 0;
    if (PIN.PID2 == 0x0A) {
...
//  LEN: 0~4096 byte : 0~1024 records (32 bit)
    ...
    DP5Proto.LISTDATA.AMPLITUDEANDTIME[0] = data_number;
    DP5Proto.LISTDATA.AMPLITUDEANDTIME[1] = DP5Proto.LISTDATA.RECORDS;
...
//          32-bit Timetag
        ...
        DP5Proto.LISTDATA.AMPLITUDEANDTIME[record * 2 + 2 + 0] = amplitude;
        DP5Proto.LISTDATA.AMPLITUDEANDTIME[record * 2 + 2 + 1] = time_tag;
    }
    data_number++;
