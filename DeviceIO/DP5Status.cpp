...
void CDP5Status::Process_Status(DP4_FORMAT_STATUS *m_DP5_Status) {
    ...
    m_DP5_Status->AccumulationTime = (float) m_DP5_Status->RAW[12] * 0.001 +
                                     (float) (m_DP5_Status->RAW[13] + (float) m_DP5_Status->RAW[14] * 256.0 +
                                              (float) m_DP5_Status->RAW[15] * 65536.0) * 0.1;
    m_DP5_Status->RealTime = ((double) m_DP5_Status->RAW[20] + ((double) m_DP5_Status->RAW[21] * 256.0) +
                              ((double) m_DP5_Status->RAW[22] * 65536.0) +
                              ((double) m_DP5_Status->RAW[23] * 16777216.0)) * 0.001;
    ...
    if (bDMCA_LiveTime) {
        m_DP5_Status->LiveTime = ((double) m_DP5_Status->RAW[16] + ((double) m_DP5_Status->RAW[17] * 256.0) +
                                  ((double) m_DP5_Status->RAW[18] * 65536.0) +
                                  ((double) m_DP5_Status->RAW[19] * 16777216.0)) * 0.001;
    } else {
    ...
    if (m_DP5_Status->RAW[30] < 128) {    // not negative
        m_DP5_Status->HV =
                ((double) m_DP5_Status->RAW[31] + ((double) m_DP5_Status->RAW[30] * 256.0)) * 0.5;  // 0.5V/count
    } else {
        m_DP5_Status->HV = (((double) m_DP5_Status->RAW[31] + ((double) m_DP5_Status->RAW[30] * 256)) - 65536.0) *
                           0.5; // 0.5V/count
    }

    m_DP5_Status->DET_TEMP =
            (double) ((m_DP5_Status->RAW[33]) + (m_DP5_Status->RAW[32] & 15) * 256) * 0.1; // - 273.16;		// 0.1K/count
    ...
    m_DP5_Status->TEC_Voltage =
            (((double) (m_DP5_Status->RAW[40] & 15) * 256.0) + (double) (m_DP5_Status->RAW[41])) / 758.5;
...
string CDP5Status::GetStatusValueStrings(DP4_FORMAT_STATUS m_DP5_Status) {
    ...
    if ((m_DP5_Status.DEVICE_ID != dppDP5G) && (m_DP5_Status.DEVICE_ID != dppTB5) &&
        (m_DP5_Status.DEVICE_ID != dppMCA8000D)) {
...
void CDP5Status::Process_Diagnostics(Packet_In PIN, DiagDataType *dd, int device_type) {
    ...
    if ((device_type == dppDP5) || (device_type == dppDP5X)) {
        for (idxVal = 0; idxVal < 10; idxVal++) {
            dd->ADC_V[idxVal] = (float) ((((PIN.DATA[5 + idxVal * 2] & 3) * 256) + PIN.DATA[6 + idxVal * 2]) * 2.44 /
                                         1024.0 * DP5_ADC_Gain[idxVal]); // convert counts to engineering units (C or V)
        }
        ...
        for (idxVal = 0; idxVal < 11; idxVal++) {
            dd->ADC_V[idxVal] = (float) ((((PIN.DATA[5 + idxVal * 2] & 15) * 256) + PIN.DATA[6 + idxVal * 2]) * 3.0 /
                                         4096.0 *
                                         PX5_ADC_Gain[idxVal]);   // convert counts to engineering units (C or V)
        }
        dd->ADC_V[11] = (float) ((((PIN.DATA[5 + 11 * 2] & 3) * 256) + PIN.DATA[6 + 11 * 2]) * 3.0 / 1024.0 *
                                 PX5_ADC_Gain[11]);  // convert counts to engineering units (C or V)
    ...
    if (dd->PC5_PRESENT) {
        for (idxVal = 0; idxVal <= 2; idxVal++) {
            dd->PC5_V[idxVal] = (float) ((((PIN.DATA[25 + idxVal * 2] & 15) * 256) + PIN.DATA[26 + idxVal * 2]) * 3.0 /
                                         4096.0 * PC5_ADC_Gain[idxVal]); // convert counts to engineering units (C or V)
        }
        if (PIN.DATA[34] < 128) {
