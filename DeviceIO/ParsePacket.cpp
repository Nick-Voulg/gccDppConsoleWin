...
long CParsePacket::ParsePacket(unsigned char P[], Packet_In *PIN) {
    ...
    if (PIN->STATUS == PID2_ACK_OK) { // no errors
        if ((PIN->PID1 == PID1_RCV_STATUS) && (PIN->PID2 == PID2_SEND_DP4_STYLE_STATUS)) { // DP4-style status
        ...
        } else if ((PIN->PID1 == PID1_RCV_SPECTRUM) && ((PIN->PID2 >= RCVPT_256_CHANNEL_SPECTRUM) && (PIN->PID2 <=
                                                                                                      RCVPT_8192_CHANNEL_SPECTRUM_STATUS))) { // spectrum / spectrum+status
            ...
            ParsePkt = preqProcessTextData;
        } else if ((PIN->PID1 == PID1_RCV_SCOPE_MISC) &&
                   (PIN->PID2 == RCVPT_SCOPE_DATA_WITH_OVERFLOW)) { //scope data with overflow packet
            //ScopeOverFlow = true;
            //ParsePkt = preqProcessScopeData;
            ParsePkt = preqProcessScopeDataOverFlow;
        } else if ((PIN->PID1 == PID1_RCV_SCOPE_MISC) &&
                   (PIN->PID2 == RCVPT_ETHERNET_SETTINGS)) { //ethernet settings packet
            //ParsePkt = preqProcessInetSettings;
        } else if ((PIN->PID1 == PID1_RCV_SCOPE_MISC) &&
                   (PIN->PID2 == RCVPT_DIAGNOSTIC_DATA)) { //diagnostic data  packet
